...
prompt = st.chat_input("Ex: Montre-moi les billets d'avion pour Lisbonne le 15 juin")
if prompt:
    ...
    with st.chat_message("assistant"):
        with st.spinner("🤔 Agent en réflexion..."):
            ...
            # Affichage Raisonnement
            if trace:
                with st.expander("🧠 Traces de raisonnement", expanded=False):
                    for i, s in enumerate(trace.get("react", []), 1):
                        st.markdown(f"**Étape {i}** 🧠 `{s.get('thought','')[:80]}...`")
                        if s.get("observation"): st.code(s["observation"][:150]+"...", "json")
            
            st.session_state.last_data = data
            if data and isinstance(data, dict):
                if data.get("outbound"):  # VOLS
                        ...
                        with tabs[idx]:
                            for i, f in enumerate(leg, 1):
                                with st.container(border=True):
                                    ...
                                    with c2: st.markdown(f"🎫 {f['cabin'].title()}")
                                    with c3: st.metric("Prix", f"{f['price']} {f['currency']}")
                    st.caption(data.get("note",""))
                    
                elif data.get("itinerary"):  # ITINÉRAIRE
                    st.markdown("### 📅 Itinéraire jour par jour")
                    for d in data["itinerary"]:
                        with st.container(border=True):
                            st.markdown(f"**Jour {d.get('day')}** | {d.get('weather','')}")
                            c1,c2,c3 = st.columns(3)
                            with c1: st.markdown(f"🌅 **Matin**\n{d.get('morning','')}")
                            with c2: st.markdown(f"☀️ **Après-midi**\n{d.get('afternoon','')}")
                            with c3: st.markdown(f"🌙 **Soirée**\n{d.get('evening','')}")
                            if d.get("notes"): st.caption(f"📌 {d['notes']}")
                    
                    md = f"# 🗺️ Itinéraire : {data.get('city','Voyage')}\n"
                    for d in data["itinerary"]:
                        md += f"\n## Jour {d.get('day')}\n- Météo: {d.get('weather')}\n- Matin: {d.get('morning')}\n- Après-midi: {d.get('afternoon')}\n- Soirée: {d.get('evening')}\n"
                    st.download_button("📥 Télécharger (.md)", md, file_name="itineraire.md", mime="text/markdown")
                else:
                    st.json(data)
